...
#define CS_H()      (GPIO_High(SD_CS_PORT, SD_CS_PIN)) /* Set MMC CS "high" */
#define CS_L()      (GPIO_Low(SD_CS_PORT, SD_CS_PIN))  /* Set MMC CS "low" */
...
static
void dly_10us (UINT n)	/* Delay 10 us (avr-gcc -Os) */
{
    SysTick_Delay(n);
}
...
{
	do {
		SPI_Read(EUSCI_B0_BASE, buff, 1);			/* Store a received byte */
		*buff++;
	} while (--bc);
}
