...
#include <ti/devices/msp432p4xx/driverlib/driverlib.h> /* Provide the low-level HAL functions */
...
#define CS_HIGH()	{GPIO_setOutputHighOnPin(GPIO_PORT_P1, GPIO_PIN0);}
#define CS_LOW()	{GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN0);}
...
void SPI_Timer_On(uint32_t waitTicks) {
    spiTimerTickStart = SysTick_getValue();
    spiTimerTickDelay = waitTicks;
}

uint8_t SPI_Timer_Status() {
    return ((SysTick_getValue() - spiTimerTickStart) < spiTimerTickDelay);
}
...
)
{
	...
	rxDat = (BYTE) SPI_receiveData(EUSCI_B0_BASE);
    return rxDat;
}
...
)
{
	...
	waitSpiTimerTickStart = SysTick_getValue();
	waitSpiTimerTickDelay = (uint32_t)wt;
	do {
		d = xchg_spi(0xFF);
		/* This loop takes a time. Insert rot_rdq() here for multitask envilonment. */
	} while (d != 0xFF && ((SysTick_getValue() - waitSpiTimerTickStart) < waitSpiTimerTickDelay));	/* Wait for card goes ready or timeout */
