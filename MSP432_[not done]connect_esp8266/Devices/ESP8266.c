...
{
    MSPrintf(EUSCI_A2_BASE, "AT+CWMODE?\r\n");
    __delay_cycles(48000000);
    MSPrintf(EUSCI_A2_BASE, "AT+CWMODE=3\r\n");
    __delay_cycles(48000000);
...
{
    MSPrintf(EUSCI_A2_BASE, "AT+CWMODE=1\r\n");
    __delay_cycles(48000000);
...
bool ESP8266_EstablishConnection(uint8_t type, char *address, uint16_t port)
{
    char ct[3];
    ...
    sprintf(test, "%s=\"%s\",\"%s\",%d\r\n",  AT_CIPSTART, ct, address, port);

    MSPrintf(EUSCI_A2_BASE, test);
